-- ERP other income print permissions (v179).
-- Safe to rerun. This script only appends menu buttons and role-menu mappings:
-- 1) print permission inherits roles that already own other income query;
...
DROP TEMPORARY TABLE IF EXISTS tmp_erp_receivable_other_income_print_permission;

CREATE TEMPORARY TABLE tmp_erp_receivable_other_income_print_permission (
...
INSERT INTO tmp_erp_receivable_other_income_print_permission
(`parent_permission`, `inherit_permission`, `button_name`, `button_permission`, `sort`)
VALUES
('erp:receivable-other-income:query', 'erp:receivable-other-income:query', '其他收入打印', 'erp:receivable-other-income:print', 7),
('erp:receivable-other-income:query', 'super_admin', '其他收入打印模板', 'erp:receivable-other-income:print-template', 8);
...
SELECT perm.button_name,
       ...
       NOW(),
       b'0'
FROM tmp_erp_receivable_other_income_print_permission perm
