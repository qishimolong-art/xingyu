-- ERP other receivable print permissions (v178).
-- Safe to rerun. This script only appends menu buttons and role-menu mappings:
-- 1) print permission inherits roles that already own other receivable query;
...
DROP TEMPORARY TABLE IF EXISTS tmp_erp_receivable_other_print_permission;

CREATE TEMPORARY TABLE tmp_erp_receivable_other_print_permission (
...
INSERT INTO tmp_erp_receivable_other_print_permission
(`parent_permission`, `inherit_permission`, `button_name`, `button_permission`, `sort`)
VALUES
('erp:receivable-other:query', 'erp:receivable-other:query', '其他应收打印', 'erp:receivable-other:print', 7),
('erp:receivable-other:query', 'super_admin', '其他应收打印模板', 'erp:receivable-other:print-template', 8);
...
SELECT perm.button_name,
       ...
       NOW(),
       b'0'
FROM tmp_erp_receivable_other_print_permission perm
