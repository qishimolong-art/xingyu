-- ERP warehouse move print permissions (v187).
-- Safe to rerun. This script only appends menu buttons and role-menu mappings:
-- 1) print permission inherits roles that already own warehouse-move query permission;
...
DROP TEMPORARY TABLE IF EXISTS tmp_erp_warehouse_move_print_permission_v187;

CREATE TEMPORARY TABLE tmp_erp_warehouse_move_print_permission_v187 (
...
INSERT INTO tmp_erp_warehouse_move_print_permission_v187
(`parent_permission`, `inherit_permission`, `button_name`, `button_permission`, `sort`)
VALUES
('erp:warehouse-move:query', 'erp:warehouse-move:query', '仓库移货单打印', 'erp:warehouse-move:print', 7),
('erp:warehouse-move:query', 'super_admin', '仓库移货单打印模板', 'erp:warehouse-move:print-template', 8);
...
SELECT perm.button_name,
       ...
       NOW(),
       b'0'
FROM tmp_erp_warehouse_move_print_permission_v187 perm
