-- Move recycle bin and parts price adjustment under ERP / system configuration.
--
-- Scope:
--   1. Ensure the ERP system-configuration directory exists.
--   2. Move existing recycle-bin and parts-price-adjustment menus under it.
--   3. Keep existing button permissions under their page menus.
--   4. Append the system-configuration parent to roles and tenant packages that
--      already own either moved menu.
--
-- Safety:
--   - No DELETE.
...
WHERE @system_config_parent_id IS NOT NULL
  AND `id` IN (@recycle_bin_menu_id, @parts_price_adjust_menu_id)
  AND `deleted` = b'0'
...
WHERE @system_config_parent_id IS NOT NULL
  ...
  AND owned_role.`menu_id` IN (@recycle_bin_menu_id, @parts_price_adjust_menu_id)
  AND NOT EXISTS (
    SELECT 1
...
WHERE @system_config_parent_id IS NOT NULL
  ...
  AND (
    ...
    OR (
      @parts_price_adjust_menu_id IS NOT NULL
