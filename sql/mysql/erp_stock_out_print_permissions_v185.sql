-- ERP stock-out print permissions (v185).
-- Safe to rerun. This script only appends menu buttons and role-menu mappings:
-- 1) print permission inherits roles that already own stock-out query permission;
...
DROP TEMPORARY TABLE IF EXISTS tmp_erp_stock_out_print_permission_v185;

CREATE TEMPORARY TABLE tmp_erp_stock_out_print_permission_v185 (
...
INSERT INTO tmp_erp_stock_out_print_permission_v185
(`parent_permission`, `inherit_permission`, `button_name`, `button_permission`, `sort`)
VALUES
('erp:stock-out:query', 'erp:stock-out:query', '其它出库单打印', 'erp:stock-out:print', 8),
('erp:stock-out:query', 'super_admin', '其它出库单打印模板', 'erp:stock-out:print-template', 9);
...
SELECT perm.button_name,
       ...
       NOW(),
       b'0'
FROM tmp_erp_stock_out_print_permission_v185 perm
