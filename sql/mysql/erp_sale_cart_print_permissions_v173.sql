-- ERP sale cart print permissions (v173).
-- Safe to rerun. This script only appends sale-cart print buttons and role-menu mappings:
-- 1) print permission inherits roles that already own sale-cart query permission;
...
DROP TEMPORARY TABLE IF EXISTS tmp_erp_sale_cart_print_permission;

CREATE TEMPORARY TABLE tmp_erp_sale_cart_print_permission (
...
INSERT INTO tmp_erp_sale_cart_print_permission
(`parent_permission`, `inherit_permission`, `button_name`, `button_permission`, `sort`)
VALUES
('erp:sale-cart:query', 'erp:sale-cart:query', '销售手推车打印', 'erp:sale-cart:print', 11),
('erp:sale-cart:query', 'super_admin', '销售手推车打印模板', 'erp:sale-cart:print-template', 12);
...
SELECT perm.button_name,
       ...
       NOW(),
       b'0'
FROM tmp_erp_sale_cart_print_permission perm
