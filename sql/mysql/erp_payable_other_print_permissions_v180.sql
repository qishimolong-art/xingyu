-- ERP other payable print permissions (v180).
-- Safe to rerun. This script only appends menu buttons and role-menu mappings:
-- 1) print permission inherits roles that already own other payable query;
...
DROP TEMPORARY TABLE IF EXISTS tmp_erp_payable_other_print_permission;

CREATE TEMPORARY TABLE tmp_erp_payable_other_print_permission (
...
INSERT INTO tmp_erp_payable_other_print_permission
(`parent_permission`, `inherit_permission`, `button_name`, `button_permission`, `sort`)
VALUES
('erp:payable-other:query', 'erp:payable-other:query', '其他应付打印', 'erp:payable-other:print', 7),
('erp:payable-other:query', 'super_admin', '其他应付打印模板', 'erp:payable-other:print-template', 8);
...
SELECT perm.button_name,
       ...
       NOW(),
       b'0'
FROM tmp_erp_payable_other_print_permission perm
