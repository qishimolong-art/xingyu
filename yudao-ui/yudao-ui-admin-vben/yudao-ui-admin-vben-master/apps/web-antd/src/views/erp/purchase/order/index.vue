...
import PurchaseInModal from './modules/purchase-in-modal.vue';
...
const purchaseInModalRef = ref<InstanceType<typeof PurchaseInModal>>();
...
/** 入库处理 */
function handlePurchaseIn(row: ErpPurchaseOrderApi.PurchaseOrder) {
  purchaseInModalRef.value?.open(row.id!);
}
...
/** 跳转字段配置页 */
function handleFieldConfig() {
  router.push({
    path: '/erp/system/fieldconfig',
    query: { moduleKey: 'purchase_order' },
  });
}
...
<template>
  <Page auto-content-height>
    ...
    <PurchaseInModal ref="purchaseInModalRef" @success="handleRefresh" />
        ...
        <TableAction
          :actions="[
            ...
            },
            {
              label: '字段配置',
              type: 'default',
              auth: ['erp:field-config:query'],
              onClick: handleFieldConfig,
            },
          ]"
        />
      </template>
        ...
        <TableAction
          :actions="[
            ...
            },
            {
              ...
              ifShow: () => row.status === 20 && (row.totalCount || 0) > (row.inCount || 0),
              onClick: handlePurchaseIn.bind(null, row),
            },
            {
