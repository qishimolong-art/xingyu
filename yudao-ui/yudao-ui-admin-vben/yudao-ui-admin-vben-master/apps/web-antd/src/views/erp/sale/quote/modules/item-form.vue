...
interface Props {
  items?: ErpSaleQuoteApi.SaleQuoteItem[];
  disabled?: boolean;
...
const tableData = ref<ErpSaleQuoteApi.SaleQuoteItem[]>([]);
...
function validate() {
  if (tableData.value.length === 0) throw new Error('请先添加报价产品');
  ...
  <Grid class="w-full">
    <template #productId="{ row }">
      <div class="flex flex-col leading-tight">
        ...
        <span v-if="row.productCode" class="text-xs text-gray-400">{{ row.productCode }}</span>
      </div>
    </template>
    <template #count="{ row }">
      <InputNumber v-if="!disabled" v-model:value="row.count" :min="0" :precision="3" @change="handleRowChange(row)" />
      <span v-else>{{ erpCountInputFormatter(row.count) || '-' }}</span>
    </template>
    <template #productPrice="{ row }">
      <InputNumber v-if="!disabled" v-model:value="row.productPrice" :min="0" :precision="2" @change="handleRowChange(row)" />
      <span v-else>{{ erpPriceInputFormatter(row.productPrice) || '-' }}</span>
    </template>
    <template #taxPercent="{ row }">
      <InputNumber v-if="!disabled" v-model:value="row.taxPercent" :min="0" :max="100" :precision="2" @change="handleRowChange(row)" />
    ...
    <template #actions="{ row }">
      <TableAction :actions="[{ label: '删除', type: 'link', danger: true, popConfirm: { title: '确认删除该产品吗？', confirm: handleDelete.bind(null, row) } }]" />
    </template>
    <template #bottom>
      <div class="mt-2 rounded border border-border bg-muted p-2">
        <div class="flex justify-between text-sm text-muted-foreground">
          ...
          <div class="flex gap-4">
            ...
            <span>金额：{{ erpPriceInputFormatter(summaries.totalPrice) }}</span>
            <span>税额：{{ erpPriceInputFormatter(summaries.taxPrice) }}</span>
          </div>
        </div>
      </div>
      <TableAction v-if="!disabled" class="mt-2 flex justify-center" :actions="[{ label: '添加报价产品', type: 'primary', onClick: handleOpenSelect }]" />
    </template>
  </Grid>
