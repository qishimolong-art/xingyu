...
@Mapper
public interface ErpPayableReportMapper extends BaseMapperX<ErpPayableOtherDO> {
    ...
    @Select({
            ...
            "              SUM(payable_amount) AS otherPayableAmount,",
            ...
            "   <when test='reqVO.orderDirection == \"asc\" and reqVO.orderField == \"settledAmount\"'>ORDER BY settledAmount ASC, s.id DESC</when>",
            "   <when test='reqVO.orderDirection == \"desc\" and reqVO.orderField == \"settledAmount\"'>ORDER BY settledAmount DESC, s.id DESC</when>",
