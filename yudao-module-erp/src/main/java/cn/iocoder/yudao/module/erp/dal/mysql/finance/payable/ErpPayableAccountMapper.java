...
public interface ErpPayableAccountMapper extends BaseMapperX<ErpPayableAccountDO> {
    ...
    @Select({
            ...
            "       IFNULL(pi.purchaseInAmount, 0) + IFNULL(pa.priceAdjustAmount, 0) + IFNULL(po.otherPayableAmount, 0) + IFNULL(pm.miscPayableAmount, 0) - IFNULL(pr.purchaseReturnAmount, 0) - IFNULL(fp.paymentAmount, 0) AS balance,",
            ...
            "   AND (IFNULL(pi.purchaseInAmount, 0) + IFNULL(pa.priceAdjustAmount, 0) + IFNULL(po.otherPayableAmount, 0) + IFNULL(pm.miscPayableAmount, 0) - IFNULL(pr.purchaseReturnAmount, 0) - IFNULL(fp.paymentAmount, 0)) &lt;&gt; 0",
            "   </if>",
            " <choose>",
    ...
    @Select({
            ...
            "          </if>",
            "       UNION ALL",
            "       SELECT '其他应付' AS docType, NULL AS bizType, id AS bizId, biz_time AS docDate, no AS docNo, false AS priceAdjusted, payable_amount AS increaseAmount, 0 AS paymentAmount, 0 AS writeOffAmount",
            "         FROM erp_payable_other",
            "        WHERE deleted = 0 AND status = 20 AND supplier_id = #{reqVO.supplierId}",
            "          <if test='reqVO.startTime != null and reqVO.endTime != null'>",
            "          AND biz_time BETWEEN #{reqVO.startTime} AND #{reqVO.endTime}",
