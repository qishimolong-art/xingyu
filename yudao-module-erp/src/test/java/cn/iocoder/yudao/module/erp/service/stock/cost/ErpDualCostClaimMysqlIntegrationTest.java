...
/** Independent shared-writer boundary: real Spring transactions, JDBC repository and calculator. Trade authorization and legacy SQL callback are fixture adapters, not a full business approval. */
@EnabledIfEnvironmentVariable(named = "ERP_REPORT_CLAIM_MYSQL_TEST_URL", matches = ".+")
class ErpDualCostClaimMysqlIntegrationTest {
    ...
    void prepareIsolatedDatabase() throws Exception {
        String url = System.getenv("ERP_REPORT_CLAIM_MYSQL_TEST_URL");
        assertTrue(url.matches("jdbc:mysql://127\\.0\\.0\\.1:33379/report_claim_test(?:\\?.*)?"),
        ...
        jdbc.update("INSERT INTO erp_stock(id,tenant_id,product_id,warehouse_id,dept_id,count,cost_price,cost_amount,deleted) VALUES (11,1,101,201,301,10,100,1000,0),(22,2,101,201,302,10,100,1000,0)");
        IsolatedStockCursorMigration.apply(jdbc,"report_claim_test");
        ...
        ReflectionTestUtils.setField(service, "tradeSnapshotService", org.mockito.Mockito.mock(cn.iocoder.yudao.module.erp.service.report.trade.ErpTradeSnapshotService.class));
    ...
    @Test void oldRepeatableReadSnapshotRetriesAlreadyExhaustedStockWithoutRecalculating()throws Exception {
        CountDownLatch snapshot=new CountDownLatch(1),committed=new CountDownLatch(1);ExecutorService pool=Executors.newSingleThreadExecutor();AtomicInteger legacyCalls=new AtomicInteger();
        try{Future<?> retry=pool.submit(()->{TenantContextHolder.setTenantId(1L);try{transaction.executeWithoutResult(s->{assertEquals(0,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_dual_cost_posting",Integer.class));snapshot.countDown();try{assertTrue(committed.await(10,TimeUnit.SECONDS));}catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException(e);}service.post(issue(1,"-10"),()->fail("duplicate may not reach legacy callback"));});}finally{TenantContextHolder.clear();}});
            assertTrue(snapshot.await(10,TimeUnit.SECONDS));post(1,issue(1,"-10"),legacyCalls);committed.countDown();retry.get(15,TimeUnit.SECONDS);assertEquals(1,legacyCalls.get());amount("quantity","0",1);amount("financial_amount","0",1);assertEquals(1,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_dual_cost_posting",Integer.class));
        }finally{committed.countDown();pool.shutdownNow();}
    }
    @Test void calculatorRejectsInsufficientStockWithoutLeavingClaimedEvent(){
        assertThrows(IllegalArgumentException.class,()->post(1,issue(1,"-11"),new AtomicInteger()));amount("quantity","10",1);amount("financial_amount","1000",1);assertEquals(0,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_dual_cost_posting",Integer.class));assertEquals(0,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_record",Integer.class));
    }
    @Test void actualCompletionUpdateFailureIsRollbackOnlyEvenWhenOuterCallerCatches(){
        jdbc.execute("CREATE TRIGGER fixture_reject_claim_completion BEFORE UPDATE ON erp_stock_dual_cost_posting FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='independent completion update failure'");
        ProxyFactory factory=new ProxyFactory(service);factory.setProxyTargetClass(true);factory.addAdvice(new TransactionInterceptor(transactionManager,new AnnotationTransactionAttributeSource()));ErpDualCostPostingService proxied=(ErpDualCostPostingService)factory.getProxy();
        try{assertThrows(UnexpectedRollbackException.class,()->transaction.executeWithoutResult(s->{try{proxied.post(issue(1,"-1"),()->jdbc.update("UPDATE erp_stock SET count=9 WHERE tenant_id=1"));}catch(org.springframework.dao.DataAccessException expected){assertTrue(expected.getMessage().contains("independent completion update failure"));}}));
            assertEquals(0,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_dual_cost_posting",Integer.class));amount("quantity","10",1);amount("financial_amount","1000",1);
        }finally{jdbc.execute("DROP TRIGGER fixture_reject_claim_completion");}
    }
    @Test void unrelatedActualUniqueConstraintCannotPretendToBeAnIdempotentSuccess(){
        jdbc.execute("ALTER TABLE erp_stock_dual_cost_posting ADD UNIQUE INDEX fixture_other_unique (biz_item_id)");
        try{post(1,issue(1,"-1"),new AtomicInteger());assertThrows(org.springframework.dao.DuplicateKeyException.class,()->post(2,issue(1,"-1"),new AtomicInteger()));assertEquals(1,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_dual_cost_posting",Integer.class));amount("quantity","10",2);amount("financial_amount","1000",2);
        }finally{jdbc.execute("ALTER TABLE erp_stock_dual_cost_posting DROP INDEX fixture_other_unique");}
    }
    @Test void olderReadSnapshotStillDetectsExternalLegacyRecordCommittedBeforeStockLock()throws Exception {
        CountDownLatch snapshot=new CountDownLatch(1),committed=new CountDownLatch(1);ExecutorService pool=Executors.newSingleThreadExecutor();
        try{Future<?> worker=pool.submit(()->{TenantContextHolder.setTenantId(1L);try{assertThrows(IllegalStateException.class,()->transaction.executeWithoutResult(s->{jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_record",Integer.class);snapshot.countDown();try{assertTrue(committed.await(10,TimeUnit.SECONDS));}catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException(e);}service.post(issue(1,"-1"),()->fail("stale legacy watermark must not reach callback"));}));}finally{TenantContextHolder.clear();}});
            assertTrue(snapshot.await(10,TimeUnit.SECONDS));jdbc.update("INSERT INTO erp_stock_record(tenant_id,product_id,warehouse_id) VALUES(1,101,201)");committed.countDown();worker.get(15,TimeUnit.SECONDS);assertEquals(0,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_dual_cost_posting",Integer.class));assertEquals(1,jdbc.queryForObject("SELECT COUNT(*) FROM erp_stock_record",Integer.class));amount("quantity","10",1);
        }finally{committed.countDown();pool.shutdownNow();}
    }
}
